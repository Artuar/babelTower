...
export const FeatureArticle: React.FC<FeatureArticleProps> = ({ title, descriptions, imagePath }) => {
  return <Box my={4}>
      ...
      <Grid item xs={12} md={8} display="flex" flexDirection="column" alignItems="flex-start">
        <Typography variant="h4" gutterBottom>{title}</Typography>
        {
          descriptions.map((description, key) => (
            <Typography variant="body1" key={key} paragraph>{description}</Typography>
          ))
        }
      </Grid>
    </Grid>
