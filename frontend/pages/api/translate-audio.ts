...
apiRoute.post(async (req: any, res: NextApiResponse) => {
  try {
    ...
    const language_from = req.body.language_from;
    const language_to = req.body.language_to;
    ...
    const response = await fetch('http://127.0.0.1:5000/api/translate-audio', {
      ...
      body: JSON.stringify({ file: `data:audio/mpeg;base64,${fileData}`, language_to, language_from, model_name }),
    });
